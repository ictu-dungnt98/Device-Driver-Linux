...
static irqreturn_t irq_handler(int irq, void* dev)
{
	...
	gpio_isr = gpio + GPIO(1);
	gpio_isr += GPIO_ISR;
	temp = read_reg(gpio_isr, 0x01 << 5);
	pr_info("clear interrupt bit\n");
	temp  |= 0x01 << 5;
	write_reg(gpio_isr, temp);
...
static int __init init_example(void)
{
	...
	res = gpio_init(gpio, iomux, GPIO(1), 5, INPUT|INTERRUPT);
	if (res == -1) {
		pr_err("Error occur when init gpio\n");
		goto gpio_init_failed;
	}
	...
	irq = gpio_to_irq(5);
	res = request_irq(irq, irq_handler, 0, "udoo_gpio5",NULL);
	if (res) {
		pr_err("Can not requrest irq\n");
		return res; 
	}
