...
{
	void __iomem *base = wdev->base;
	printk(KERN_EMERG "Dungnt98 %s, %d, base = %p", __func__, __LINE__, base);
	...
	while (readl_relaxed(base + OMAP_WATCHDOG_WPS) & 0x10)
		cpu_relax();
	printk(KERN_EMERG "Dungnt98 %s, %d, reg = %p", __func__, __LINE__, (base + OMAP_WATCHDOG_WPS));

	writel_relaxed(0x5555, base + OMAP_WATCHDOG_SPR);	/* TIMER_MODE */
	printk(KERN_EMERG "Dungnt98 %s, %d, reg = %p", __func__, __LINE__, (base + OMAP_WATCHDOG_SPR));
...
static int omap_wdt_probe(struct platform_device *pdev)
{
	...
	printk(KERN_EMERG "Dungnt98 %s, %d, base = %p", __func__, __LINE__, wdev->base);
	...
	if (watchdog_init_timeout(&wdev->wdog, timer_margin, &pdev->dev) < 0) {
		...
		printk(KERN_EMERG "Dungnt98 %s, %d", __func__, __LINE__);
	}
